 ...
 *   - Isolation par client (correctif M2) : la clé intègre un `scope` dérivé
 *     de l'identifiant/crédential présenté. Deux clients (même clé partagée ou
 *     clés distinctes) ne se servent plus mutuellement leurs réponses cachées.
 *
 * Variables d'env :
 ...
 *
 * `scope` (optionnel, défaut "default") : identifiant de client/tenant qui
 * isole les entrées de cache entre consommateurs. Le router le dérive du
 * crédential d'authentification présenté (voir api/chat.js, api/v1-chat.js,
 * api/normalize.js).
 */
function computeKey({ messages, modelOverrides = {}, onlyProviders = null, scope = "default" }) {
