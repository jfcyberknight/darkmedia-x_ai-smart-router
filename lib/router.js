...
const cliBridge = require("./providers/cli-bridge");
...
const PROVIDERS = [
  ...
  {
    id: "cli-bridge",
    generate: cliBridge.generate,
    apiKeyEnv: "CLI_BRIDGE_KEY", // Optionnel — le pont peut fonctionner sans clé
    defaultModel: cliBridge.DEFAULT_MODEL,
    noKeyRequired: true, // Toujours dispo (pas besoin de clé API)
  },
];
...
async function routeChat({ messages, modelOverrides = {}, onlyProviders = null, clientScope }) {
  ...
  let available = PROVIDERS.filter((p) => {
    ...
    // Cas particulier cli-bridge : toujours dispo (pas besoin de clé API)
    if (p.noKeyRequired) return true;
  ...
  for (const provider of order) {
    ...
    // Ollama et cli-bridge peuvent ne pas avoir de clé
    if (!apiKey && !provider.noKeyRequired && provider.id !== "ollama") {
