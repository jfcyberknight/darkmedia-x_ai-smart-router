 ...
 * En cas d'erreur (402 insufficient balance, 429 quota, 500, 503, etc.), tente le suivant.
 */
async function routeChat({ messages, modelOverrides = {} }) {
  const available = PROVIDERS.filter((p) => process.env[p.apiKeyEnv]);
  ...
  for (const provider of order) {
    ...
    try {
      ...
      const isRetryable =
        status === 402 || status === 429 || status === 500 || status === 503 ||
        /quota|rate limit|insufficient balance/i.test(err.message || '');
