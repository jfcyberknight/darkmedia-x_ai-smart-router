 ...
 * URL par défaut: https://ai-smart-router.vercel.app (ou API_BASE_URL dans .env)
 * Lit .env pour API_SECRET.
 */
...
async function testHealth() {
  ...
  if (
    data.statut !== "actif" ||
    !data.donnees ||
    data.donnees.ok !== true ||
    !Array.isArray(data.donnees.providers)
  ) {
    throw new Error(
      `Health: format invalide (envelope statut/donnees attendu): ${JSON.stringify(data)}`
    );
  }
  return data.donnees;
}
...
async function testChat() {
  ...
  if (
    ...
    throw new Error(
      `Chat: format invalide (content, provider, model attendus): ${JSON.stringify(Object.keys(data))}`
    );
  }
  return payload;
}
...
async function run() {
  if (!apiSecret) {
    console.error("❌ API_SECRET manquant dans .env (requis pour l’API déployée).");
    process.exit(1);
  }
  ...
  try {
    ...
    console.log(
      "   content:",
      (chat.content || "").trim().slice(0, 120) + (chat.content?.length > 120 ? "…" : ""),
      "\n"
    );
    ...
    if (err.message.includes("404")) {
      ...
      console.error(
        "   → Vercel > Project Settings > General : Framework Preset = Other (pas Next.js)."
      );
    }
    process.exit(1);
  }
